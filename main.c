...
int main(int argc, const char * argv[])
{
   ...
   */
   ...
   if(argc==2)
    ...
    while(workingCandidate.numberOfDefinedValues < N ) {
        
        AddQueenToNextRowInColumn(0);
        AddWorkingCandidateToOpenList();
        
        for(int col = 0; col < N; col++) {
            MoveQueenInRowToNewCol(workingCandidate.numberOfDefinedValues - 1, col);
            AddWorkingCandidateToOpenList();
        }
        
        while(currentListOfCandidates.indexOfLastEntryAdded >= 0) {
            CalculateNumberOfVulnerableQueensForWorkingCandidate();
            
            if (workingCandidate.score == 0) { break; }
            
            int last = currentListOfCandidates.indexOfLastEntryAdded;
            CopySolutionFromCurrentListIntoWorkingCandidate(last);
            RemoveSolutionFromCurrentList(last);
        }
